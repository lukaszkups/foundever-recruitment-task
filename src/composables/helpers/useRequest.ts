import { AxiosMethods, TBuildRequestConstructorParams, TBuildRequestContructorValues, TMovieApiRequestValues, TUseRequestResult } from "@/types/api"
...
export default (
	...
	}: TMovieApiRequestValues = {},
): TUseRequestResult | boolean => {
	try {
		return buildRequest(
			{
				api,
...
const buildRequest = (
	{ api, method, path: path, params: paramsBuilder, query: queryBuilder, body: bodyBuilder, config: configBuilder }: TBuildRequestConstructorParams,
...
const requestBuilder = (
	...
	bodyValue: TMovieApiRequestValues | null = null, 
	config: TBuildRequestConstructorParams['config'] | TBuildRequestContructorValues['config'] | Record<string, any> = {}
) => {
	if (config.headers) {
		config.headers = {
	...
	return {
		...
		exec: [ METHOD.GET, METHOD.DELETE ].includes(method)
			? () => (api as unknown as Record<AxiosMethods | string, Function>)[method](pathValue, config)
			: () => (api as unknown as Record<AxiosMethods | string, Function>)[method](pathValue, bodyValue, config),
	}
}
